...
class WorkflowItemModel extends Model
{
    ...
    {
        return $this->handle($data, $adminId,2);
    }
    ...
    public function handle(array $data, $adminId,$result){
        try {
            ...
            $workflowItemProcessModel->updateProcess(['itemId' => $id, 'adminId' => $adminId], ['updateTime' => date('Y-m-d H:i:s'), 'status' => $result, 'remark' => $remark]);
            //更新审批的状态
     ...
     * @return mixed
     */
    public function myApprovalLists(array $input)
    {
        $limit = config('yucheng.limit');
        $start = is_null($input['start']) ? 0 : $input['start'];
        ...
        return DB::table($this->table)
            ...
            ->leftJoin('admin as a','a.id','=',$this->table.'.adminId')
            ->where(function ($query) use ($input){
                if (isset($input['adminId']) && !empty($input['adminId'])){
                    $query->where($this->table.'.adminId',$input['adminId']);
                }
                if (isset($input['startTime']) && !is_null($input['startTime'])){
                    $query->where('joinTime','>=',$input['startTime'].' 00:00:00');
                }
     ...
     * @return mixed
     */
    public function myApprovalCountLists(array $input)
    {
        return DB::table($this->table)
            ->leftJoin('workflow as wf','wf.id','=',$this->table.'.workflowId')
            ->leftJoin('project as p','p.id','=',$this->table.'.projectId')
